...
### 指令域
指触发该指令的环境，比如一条群指令，在私聊情况下是不会触发bot的，指令域有:私聊，群，任何人
### 权限
...
|测试|hello|测试bot是否启动成功|任何|任何人|

### 自带小功能
| 功能 | 指令 | 描述 | 指令域 | 权限 |
| :-----| :---- | :---- | :---- | :---- |
|每日抽签|抽签/gacha|每天一次抽签测运气，重复抽签不会改变世界线哦|群|任何人|
|roll点|roll [数字 可选]|随机roll一个 0~输入数字，省略输入则为 1~100|群|任何人|
|计划表 创建|plan/计划 [new/新建/创建] [目标名] [目标数量]|创建一个计划任务，比如我今天要俯卧撑 100个|群|任何人|
|计划表 增量更新|plan/计划 [add/增加] [目标名] [增加数量]|增量更新计划，比如我刚刚做了10个俯卧撑，用本指令增加10|群|任何人|
|计划表 直接更新|plan/计划 [update/更新] [目标名] [目前数量]|直接更新计划，中间增加了多少次记不清了，但是现在已经做了90个俯卧撑，使用本指令|群|任何人|
|计划表 查询目标|plan/计划 [check/look/查询] [目标名]|看一下我今天做了几个俯卧撑了|群|任何人|
|计划表 查询所有目标|plan/计划|看看我今天一共立了多少flag|群|任何人|
|计划表 删除|plan/计划 [delete/del/删除] [目标名]|销毁flag，今天不俯卧撑了！|群|任何人|
|编码 朝代|加密/帮我加密 [明文]|本质是编码后用字典替换编码，而非加密。由于替换后是 夏商周... 所以称为朝代，替换源码中的自定义编码表 可以个性化，比如换成 动物，颜色等等|群|任何人|
|解码 朝代|解密/帮我解密 [密文]|同上 将编码还原为原本文字|群|任何人|
...
|jjc查询|jjc查询 [角色1] [角色2] ...|jjc查询 目前默认国服不可更改，返回文字版作业|任何|任何人|
